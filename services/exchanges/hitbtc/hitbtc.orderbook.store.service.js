  ...
  function wsService(orderBookCollection, market) {
    ...
    return new Promise((resolve, reject) => {
      ...
      rws.addEventListener('message', event => {
        ...
        if (typeof(newData.params) !== "undefined") {
          
          if (sequenceNum === 0) {
            sequenceNum = newData.params.sequence - 1
          }
          
          if (newData.params.sequence - 1 === sequenceNum) {
            sequenceNum = newData.params.sequence;
      ...
      rws.addEventListener('close', () => {
        books.deleteMany({exchange: exchange, market: market})
          .then(() => {
            wsService(orderBookCollection, market)
              .catch(err => {
                console.log(err + '\nError: Exiting...')
                process.exit(1)
              });
